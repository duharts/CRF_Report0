...
st.write(df[['Facility', 'Units Offline', 'Daily Rate ($)', 'Days Offline', 'Revenue Lost Due to Offline Units']])
...
st.write(df[['Facility', 'Units Offline', 'Repair Costs ($)']])
...
st.write(df[['Facility', 'Cribs', 'Total Units', 'Crib Utilization (%)']])
...
**Executive Summary**:  
**House East
Ensuring crib availability can help maintain this demand and optimize family-focused housing.
""")
...
st.write(df[['Facility', 'Units Offline', 'Return to Full Capacity (Days)']])
...
st.markdown("""
**Executive Summary**:  
**Light House** and **Hope House** will require longer periods to return to full capacity due to a significant number of offline units.
""")
